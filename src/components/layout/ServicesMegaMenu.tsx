...
const ServicesMegaMenu: React.FC<ServicesMegaMenuProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="absolute top-full left-0 w-screen bg-white shadow-2xl border-t z-50 animate-in slide-in-from-top-2 duration-300">
                        ...
                        {category.services && category.services.length > 0 ? (
                          <>
                            {category.services.slice(0, 6).map((service) => (
                              <Link
                                key={service.id}
                                to={`/services/${slugify(category.title)}/${slugify(service.title)}`}
                                onClick={onClose}
